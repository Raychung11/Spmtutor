<?php
/**
 * Demo data seeder. Creates sample students, attempts, subscriptions, a
 * class, assignments, Snap & Check uploads and parent reports so the
 * dashboards and analytics look populated.
 *
 * Run from CLI:   php public_html/cron/seed_demo.php
 *                 php public_html/cron/seed_demo.php --force
 * Or from the admin panel: Admin → Seeders → "Demo data".
 */
...
function seed_demo_data_run(bool $force = false): array
{
    ...
    if ($marker && $marker['setting_value'] === '1' && !$force) {
        return ['status' => 'already_seeded', 'message' => 'Demo data already seeded. Pass force to re-run.'];
    }
    ...
    if (!$questions) {
        return ['status' => 'no_questions', 'message' => 'No questions found — seed courses first.'];
    }
    ...
    return [
        ...
        'message'  => 'Demo data seeded — ' . count($studentIds) . ' students, 1 parent, 1 teacher, 1 class.',
    ];
}
...
if (PHP_SAPI === 'cli') {
    ...
    echo $r['message'] . "\n";
    if (($r['status'] ?? '') === 'seeded') {
        echo "Logins — students: student1@demo.lulusai.my .. student6@... | parent@demo... | teacher@demo... (password: Demo@123)\n";
    }
}
